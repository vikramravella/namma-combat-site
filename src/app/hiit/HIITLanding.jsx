...
export default function HIITLanding() {
  const [v, setV] = useState(false);
  useEffect(() => { setV(true); }, []);
  const levels = [
    {
      label: 'For the Beginner',
      intro: "You want to get fit fast. Structure matters more than intensity.",
      items: [
        'Work-to-rest ratios that actually match your fitness level',
        'Form under fatigue — bad form at high intensity is how injuries happen',
        'Introduction to heart rate zones — Zone 2, Zone 4, Zone 5',
        'Bike and rower fundamentals — Technogym Skill Row, Skill Ski, air bikes',
        'Progressive intensity — we build capacity before we push the ceiling',
      ],
    },
    {
      label: 'For the Intermediate',
      intro: 'You want real conditioning. Not cardio theatre.',
      items: [
        'HIIT formats done right — Tabata (20:10), EMOM, AMRAP, intervals',
        'Energy system targeting — ATP-PC, glycolytic, oxidative',
        'V̇O₂ max work, lactate threshold training, tempo efforts',
        'Sport-specific conditioning for combat athletes, runners, lifters',
        'Heart rate data to guide your work — not guess',
      ],
    },
    {
      label: 'For the Athlete',
      intro: 'You compete. Your conditioning needs to peak at the right time.',
      items: [
        'Block-periodised conditioning aligned with your sport calendar',
        'In-season maintenance vs off-season capacity building',
        'Specific work capacity — round-length for combat, race-length for endurance',
        'Recovery protocols — HRV, sleep, load management',
        'Benchmarks and retests — proof your conditioning is actually improving',
      ],
    },
  ];
  return (
        ...
        <div className="nc-landing-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 48, alignItems: 'start' }}>
          <div>
            ...
            <div style={{ marginBottom: 36 }}>
              ...
              <h2 style={{ fontFamily: T.fontDisplay, fontSize: 24, fontWeight: 900, color: T.text, margin: '0 0 8px', textTransform: 'uppercase', letterSpacing: 0.5 }}>HIIT at every level.</h2>
              ...
              <div style={{ marginTop: 24, padding: '18px 22px', background: 'rgba(227,199,104,0.12)', borderLeft: `3px solid ${T.gold}`, borderRadius: 4 }}>
                <h3 style={{ fontFamily: T.fontDisplay, fontSize: 13, fontWeight: 900, color: T.text, margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: 1 }}>Equipment that earns its name</h3>
                <p style={{ fontFamily: T.fontBody, fontSize: 13, color: T.textLight, lineHeight: 1.6, margin: 0 }}>Hammer Strength air bikes that punish you in the right way. Technogym Skill Row and Skill Ski for upper-body dominant intervals. Dedicated rowers, bikes, and open floor space. Not treadmills dressed up as HIIT — proper conditioning tools used correctly.</p>
              </div>
              <div style={{ marginTop: 16, padding: '18px 22px', background: T.warm, borderLeft: `3px solid ${T.rust}`, borderRadius: 4 }}>
                ...
                <p style={{ fontFamily: T.fontBody, fontSize: 13, color: T.textLight, lineHeight: 1.6, margin: 0 }}><strong style={{ color: T.text }}>Coach Mohammed Naeem</strong> — MSc Performance Coaching (Setanta College), 8+ years programming energy system work for athletes. Supported by the full Namma Combat S&C team — every HIIT session is structured, assessed, and matched to your capacity.</p>
              </div>
            </div>
