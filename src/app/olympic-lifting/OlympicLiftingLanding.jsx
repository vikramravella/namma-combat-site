...
export default function OlympicLiftingLanding() {
  const [v, setV] = useState(false);
  useEffect(() => { setV(true); }, []);
  const levels = [
    {
      label: 'For the Beginner',
      intro: "You've never lifted these. That's where we start — properly.",
      items: [
        'Overhead squat assessment — before you snatch, we see where your body actually is',
        'Bar path fundamentals — the bar stays close, always',
        'The progression: PVC pipe → empty bar → loaded bar',
        'Positions first — high hang, hang, floor (in that order)',
        'Power variants before full variants — safer, teaches the pull',
        'Structured S&C for mobility work — shoulders, hips, ankles ready to catch',
      ],
    },
    {
      label: 'For the Intermediate',
      intro: 'The positions are there. Now we build the full lift.',
      items: [
        'Full snatch — squat snatch from the floor with correct timing',
        'Full clean & jerk — squat clean, split jerk, locked overhead',
        'Pulling derivatives — high pulls, clean pulls, snatch pulls, segment pulls',
        'Complexes — clean + front squat + jerk; snatch + overhead squat',
        'Programming by percentage — not by feel, not by ego',
        'Weak point identification — first pull, transition, catch, recovery',
      ],
    },
    {
      label: 'For the Competitor',
      intro: 'You want to compete. We prepare you for the platform.',
      items: [
        'Peaking cycles — accumulation, intensification, realisation blocks',
        'Percentage-based programming with max-out protocols',
        'Competition commands — rack position, jerk, down signal',
        'Weight class strategy and attempt selection',
        'IWF / IWLF / state-level competition prep',
        'Recovery — sleep, nutrition, load management through the peak',
      ],
    },
  ];
  return (
        ...
        <div className="nc-landing-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 48, alignItems: 'start' }}>
          <div>
            ...
            <div style={{ marginBottom: 36 }}>
              ...
              <h2 style={{ fontFamily: T.fontDisplay, fontSize: 24, fontWeight: 900, color: T.text, margin: '0 0 8px', textTransform: 'uppercase', letterSpacing: 0.5 }}>Olympic lifting at every level.</h2>
              ...
              <div style={{ marginTop: 24, padding: '18px 22px', background: 'rgba(227,199,104,0.12)', borderLeft: `3px solid ${T.gold}`, borderRadius: 4 }}>
                <h3 style={{ fontFamily: T.fontDisplay, fontSize: 13, fontWeight: 900, color: T.text, margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: 1 }}>The right equipment</h3>
                <p style={{ fontFamily: T.fontBody, fontSize: 13, color: T.textLight, lineHeight: 1.6, margin: 0 }}>Custom stainless steel squat racks. Competition-spec bars and bumper plates. Dedicated Olympic lifting platforms — not a corner of a commercial gym. Equipment built for the lifts, coaches trained to teach them.</p>
              </div>
              <div style={{ marginTop: 16, padding: '18px 22px', background: T.warm, borderLeft: `3px solid ${T.rust}`, borderRadius: 4 }}>
                ...
                <p style={{ fontFamily: T.fontBody, fontSize: 13, color: T.textLight, lineHeight: 1.6, margin: 0 }}><strong style={{ color: T.text }}>Coach Mohammed Naeem</strong> — MSc Performance Coaching (Setanta College), 8+ years coaching the Olympic lifts with pro and amateur athletes. Every lift is taught position-by-position, filmed when needed, coached through every plate.</p>
              </div>
            </div>
