...
function InlineForm() {
  const [fd, setFd] = useState({ name: '', phone: '', interest: 'MMA - Mixed Martial Arts', notes: '' });
...
export default function MMALanding() {
  ...
  const levels = [
    ...
    },
    {
      ...
      items: [
        'Dirty boxing in the clinch — short hooks, elbows, head position battles',
        'Sprawl-and-brawl — takedown defence chained with counter-striking',
        'Submission defence from guard and escapes from disadvantageous positions',
        'Cage-assisted techniques — using the fence to stand, break grips, reset',
        'Ground-and-pound from mount, side control, and back positions',
        'Live MMA sparring with controlled intensity and integrated conditioning',
      ],
    },
    {
      ...
      items: [
        'Cage cutting — controlling cage position, denying retreat, creating angles',
        'MMA submission chains — transitioning between submissions under strike threat',
        'Five-round championship conditioning — energy system work for 25-minute fights',
        'Opponent-specific game planning — film study, matchup analysis, tactical adjustments',
        'Fight camp periodisation — peaking strength, volume tapers, active recovery',
        'Weight cut protocols, hydration strategy, and fight-night corner work',
      ],
    },
  ];
  return (
        ...
        <div className="nc-landing-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 48, alignItems: 'start' }}>
          <div>
            ...
            <div style={{ marginBottom: 36 }}>
              ...
              <div style={{ marginTop: 24, padding: '18px 22px', background: 'rgba(227,199,104,0.12)', borderLeft: `3px solid ${T.gold}`, borderRadius: 4 }}>
                ...
                <p style={{ fontFamily: T.fontBody, fontSize: 13, color: T.textLight, lineHeight: 1.6, margin: 0 }}>Every MMA athlete here trains S&C. Non-negotiable at every level. An MMA athlete's body takes more stress than any other sport — we build bodies that last camps, not just classes.</p>
              </div>
              <div style={{ marginTop: 16, padding: '18px 22px', background: T.warm, borderLeft: `3px solid ${T.rust}`, borderRadius: 4 }}>
                ...
                <p style={{ fontFamily: T.fontBody, fontSize: 13, color: T.textLight, lineHeight: 1.6, margin: 0 }}><strong style={{ color: T.text }}>Coach Kantharaj Agasa</strong> — Indian MMA Pioneer, 12 professional MMA wins, national-level wrestler, NIS Patiala certified Judo coach, Gold medals in BJJ and Judo. Supported by Coach Bhagyarajan (boxing), Coach Venkatesh (wrestling), Coach Naeem (S&C).</p>
              </div>
            </div>
          ...
          <div style={{ position: 'sticky', top: 24, opacity: v?1:0, transform: v?'translateY(0)':'translateY(20px)', transition: 'all 0.6s ease 0.3s' }}>
            <div style={{ background: '#fff', borderRadius: 12, padding: '32px 28px', border: `1px solid ${T.border}`, boxShadow: '0 4px 24px rgba(44,35,24,0.06)' }}>
              ...
              <p style={{ fontFamily: T.fontBody, fontSize: 13, color: T.textLight, margin: '0 0 24px' }}>We&apos;ll call to schedule your complimentary MMA trial.</p>
              <InlineForm />
            </div>
